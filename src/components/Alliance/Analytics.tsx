...
export function Analytics({ alliance }: AnalyticsProps) {
	...
	if (isLoadingAnalytics || isLoadingScanDates) {
		return (
	...
	const powerDistribution = powerRanges.map((range) => ({
		range: range.range,
		count: analyticsData.filter(
			(p) => (p.power || 0) >= range.min && (p.power || 0) < range.max
		).length,
		color: range.color,
	}));

	// Faction distribution
	const factionStats = analyticsData.reduce((acc, player) => {
	...
	// Top performers data
	const topPowerPlayers = [...analyticsData]
	...
	const topMeritPlayers = [...analyticsData]
	...
	const totalPower = analyticsData.reduce((sum, p) => sum + (p.power || 0), 0);
	const totalMerits = analyticsData.reduce(
		(sum, p) => sum + (p.merits || 0),
		0
	);
	const totalKills = analyticsData.reduce(
		(sum, p) => sum + (p.units_killed || 0),
		0
	);
	const avgPower =
		analyticsData.length > 0 ? totalPower / analyticsData.length : 0;
			...
			<div className="space-y-6">
				...
				<div className="grid grid-cols-1 md:grid-cols-4 gap-6">
					...
					>
						...
						<p className="text-xs text-cyan-400 font-bold relative z-10">
							Avg: {formatNumber(avgPower)}
						</p>
					</motion.div>
							...
							<ResponsiveContainer width="100%" height="100%">
								<PieChart>
									<Pie
										...
										cy="50%"
										labelLine={false}
										label={({ faction, percent }: any) =>
											`${faction} ${(percent * 100).toFixed(0)}%`
										}
										outerRadius={80}
										fill="#8884d8"
										dataKey="count"
									>
										{factionData.map((_, index) => (
											<Cell
												key={`cell-${index}`}
				...
				>
					...
					<div className="space-y-6 relative z-10">
						...
						<div className="glass-panel-light rounded-xl p-6 neon-border bg-gradient-to-r from-blue-500/10 to-purple-500/10">
							<h5 className="text-md font-bold text-blue-400 font-orbitron mb-4">
								📊 TOTAL POWER CALCULATION
							</h5>
							<div className="space-y-3 font-mono text-sm">
								<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
									...
									<span className="text-white">
										Sum of all player power values
									</span>
								</div>
								<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
									<span className="text-purple-400">Players Count:</span>
									<span className="text-white">
										{analyticsData.length} active members
									</span>
								</div>
								<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
								...
								<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
									...
									<span className="text-white">
										{totalPower.toLocaleString()} ÷ {analyticsData.length} ={" "}
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
							<div className="glass-panel-light rounded-xl p-6 neon-border bg-gradient-to-r from-purple-500/10 to-pink-500/10">
								<h5 className="text-md font-bold text-purple-400 font-orbitron mb-4">
									🏆 MERIT CALCULATION
								</h5>
								<div className="space-y-3 font-mono text-sm">
									<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
									...
									<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
										...
										<span className="text-white">
											{Math.round(
												totalMerits / analyticsData.length
											).toLocaleString()}
										</span>
									</div>
								</div>
							...
							<div className="glass-panel-light rounded-xl p-6 neon-border bg-gradient-to-r from-red-500/10 to-orange-500/10">
								<h5 className="text-md font-bold text-red-400 font-orbitron mb-4">
									⚔️ KILL CALCULATION
								</h5>
								<div className="space-y-3 font-mono text-sm">
									<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
									...
									<div className="flex items-center justify-between bg-gray-800/50 p-3 rounded-lg">
										...
										<span className="text-white">
											{Math.round(
												totalKills / analyticsData.length
											).toLocaleString()}
										</span>
									</div>
								</div>
						...
						<div className="glass-panel-light rounded-xl p-6 neon-border bg-gradient-to-r from-green-500/10 to-teal-500/10">
							<h5 className="text-md font-bold text-green-400 font-orbitron mb-4">
								📈 POWER DISTRIBUTION ANALYSIS
							</h5>
							<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
								{powerDistribution.map((range) => (
									<div
										key={range.range}
									...
									>
										...
										<div className="space-y-2 font-mono text-xs">
											...
											<div className="flex justify-between">
												...
												<span className="text-white">
													{((range.count / analyticsData.length) * 100).toFixed(
														1
													)}
													%
												</span>
											...
											<div className="flex justify-between">
												...
												<span className="text-white text-xs">
													({range.count} ÷ {analyticsData.length}) × 100
												</span>
											</div>
										</div>
									</div>
						...
						<div className="glass-panel-light rounded-xl p-6 neon-border bg-gradient-to-r from-yellow-500/10 to-orange-500/10">
							<h5 className="text-md font-bold text-yellow-400 font-orbitron mb-4">
								📋 STATISTICAL SUMMARY
							</h5>
							<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
								<div className="space-y-3">
									<h6 className="text-sm font-bold text-cyan-400 font-orbitron">
										AGGREGATION METHODS
									</h6>
									<div className="space-y-2 text-sm">
										...
										<div className="flex justify-between bg-gray-800/50 p-2 rounded">
											...
											<span className="text-white font-mono">
												COUNT(range) ÷ TOTAL × 100
											</span>
										</div>
									</div>
								</div>
								<div className="space-y-3">
									<h6 className="text-sm font-bold text-purple-400 font-orbitron">
										DATA SOURCES
									</h6>
									<div className="space-y-2 text-sm">
										<div className="flex justify-between bg-gray-800/50 p-2 rounded">
											...
											<span className="text-white">
												Players with 50M+ highest_power
											</span>
										</div>
										<div className="flex justify-between bg-gray-800/50 p-2 rounded">
											<span className="text-gray-400">Filter:</span>
											<span className="text-white">
												home_server IS NOT NULL
											</span>
										</div>
										<div className="flex justify-between bg-gray-800/50 p-2 rounded">
				...
				>
					...
					<div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
						<div className="text-center p-4 glass-panel-light rounded-xl neon-border">
							<p className="text-sm text-blue-400 font-bold mb-2 font-orbitron">
								POWER PER MEMBER
							</p>
							<p className="text-xl font-black text-white font-orbitron">
								{formatNumber(avgPower)}
							</p>
						</div>
						<div className="text-center p-4 glass-panel-light rounded-xl neon-border">
							<p className="text-sm text-purple-400 font-bold mb-2 font-orbitron">
								MERIT PER MEMBER
							</p>
							<p className="text-xl font-black text-white font-orbitron">
								{formatNumber(totalMerits / analyticsData.length)}
							</p>
						</div>
						<div className="text-center p-4 glass-panel-light rounded-xl neon-border">
							<p className="text-sm text-red-400 font-bold mb-2 font-orbitron">
								KILLS PER MEMBER
							</p>
							<p className="text-xl font-black text-white font-orbitron">
								{formatNumber(totalKills / analyticsData.length)}
							</p>
						</div>
					</div>
				</motion.div>
