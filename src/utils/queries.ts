import { AllianceRanking } from "../hooks/useAllianceRankings";
...
export async function fetchAllianceDetail(
	allianceId: string
	...
	const { data: alliance, error: allianceError } = await supabase
		.from("alliances")
		.select(
			"id, tag, name, generation, season_start_date, season_end_date, created_at"
		)
		.eq("id", allianceId)
		.single();
	...
	if (latestScan && !scanError) {
		...
		if (!scanIdError && scanData) {
			...
			if (!statsError && stats) {
				...
				averagePower =
					memberCount > 0 ? Math.round(totalPower / memberCount) : 0;
			}
		}
	}
...
export async function fetchLeaderboard(
	...
	return (data || []).map((stat) => ({
		...
		alliance_tag: stat.alliance_tag || null,
	}));
}
...
export async function fetchPlayerRankings(
	...
	Record<string, { rank: number; total: number; percentile: number }>
> {
	// Get the latest scan for this alliance
	const { data: latestScan, error: scanError } = await supabase
		.from("scans")
	...
	fieldsToRank.forEach((field) => {
		...
		const playersWithHigherValues = allStats.filter(
			(stat) => ((stat as any)[field] || 0) > playerValue
		).length;
...
export async function fetchAllianceDateRange(
	allianceId: string
): Promise<{ startDate: string; endDate: string; scanCount: number }> {
	const { data, error } = await supabase
		.from("scans")
		.select("scan_date")
		.eq("alliance_id", allianceId)
...
export async function fetchAllianceScanDates(
	allianceId: string
): Promise<string[]> {
	const { data, error } = await supabase
		.from("scans")
...
export async function fetchLeaderboardForDate(
	allianceId: string,
	scanDate: string
): Promise<LeaderboardEntry[]> {
	// Filter by 50M+ power instead of using limit
...
export async function fetchOverviewStats(
	...
	const { data: latestStats, error: latestError } = await supabase
		.from("player_stats")
		.select(
			"power, highest_power, merits, units_killed, killcount_t5, mana_spent, units_dead, home_server"
		)
	...
	const totalPower =
		latestStats?.reduce((sum, stat) => sum + (stat.power || 0), 0) || 0;
	const totalMerits =
		latestStats?.reduce((sum, stat) => sum + (stat.merits || 0), 0) || 0;
	const totalKills =
		latestStats?.reduce((sum, stat) => sum + (stat.units_killed || 0), 0) || 0;
	const totalT5Kills =
		latestStats?.reduce((sum, stat) => sum + (stat.killcount_t5 || 0), 0) || 0;
	const totalManaSpent =
		latestStats?.reduce((sum, stat) => sum + (stat.mana_spent || 0), 0) || 0;
	const totalUnitsDead =
		latestStats?.reduce((sum, stat) => sum + (stat.units_dead || 0), 0) || 0;
	const averagePower =
		playerCount > 0 ? Math.round(totalPower / playerCount) : 0;
	...
	if (scans.length > 1) {
		...
		const { data: previousStats, error: previousError } = await supabase
			.from("player_stats")
			.select(
				"power, highest_power, merits, units_killed, mana_spent, units_dead, home_server"
			)
		...
		if (!previousError && previousStats) {
			const previousTotalPower = previousStats.reduce(
				(sum, stat) => sum + (stat.power || 0),
				0
			);
			const previousTotalMerits = previousStats.reduce(
				(sum, stat) => sum + (stat.merits || 0),
				0
			);
			const previousTotalKills = previousStats.reduce(
				(sum, stat) => sum + (stat.units_killed || 0),
				0
			);
			const previousTotalManaSpent = previousStats.reduce(
				(sum, stat) => sum + (stat.mana_spent || 0),
				0
			);
			const previousTotalUnitsDead = previousStats.reduce(
				(sum, stat) => sum + (stat.units_dead || 0),
				0
			);

			powerChange = totalPower - previousTotalPower;
			powerChangePercent =
				previousTotalPower > 0 ? (powerChange / previousTotalPower) * 100 : 0;

			meritChange = totalMerits - previousTotalMerits;
			meritChangePercent =
				previousTotalMerits > 0 ? (meritChange / previousTotalMerits) * 100 : 0;

			killChange = totalKills - previousTotalKills;
			killChangePercent =
				previousTotalKills > 0 ? (killChange / previousTotalKills) * 100 : 0;

			manaSpentChange = totalManaSpent - previousTotalManaSpent;
			manaSpentChangePercent =
				previousTotalManaSpent > 0
					? (manaSpentChange / previousTotalManaSpent) * 100
					: 0;

			unitsDeadChange = totalUnitsDead - previousTotalUnitsDead;
			unitsDeadChangePercent =
				previousTotalUnitsDead > 0
					? (unitsDeadChange / previousTotalUnitsDead) * 100
					: 0;
		}
	}
 ...
 *
 */
export async function fetchAllianceRankings(): Promise<AllianceRanking[]> {
	// Get all alliances
	...
	// Fetch all latest scans for all alliances in parallel
	const scanPromises = alliances.map((alliance) =>
		supabase
			.from("scans")
			.select("id, scan_date")
	...
	// Create a map of alliance ID to scan
	const scanMap = new Map(
		scanResults
			.filter((r) => r.scan && !r.error)
			.map((r) => [r.allianceId, r.scan!])
	);

	// Fetch stats for all scans in parallel
	const statsPromises = Array.from(scanMap.entries()).map(
		([allianceId, scan]) =>
			supabase
				.from("player_stats")
				.select(
        ...
        home_server`
				...
				.order("highest_power", { ascending: false })
				.then((result) => ({
					allianceId,
					stats: result.data,
	...
	for (const alliance of alliances) {
		const statsResult = statsResults.find((r) => r.allianceId === alliance.id);

		if (!statsResult || !statsResult.stats || statsResult.error) {
			continue;
		}
		...
		const totalMerits = stats.reduce(
			(sum, stat) => sum + (stat.merits || 0),
			0
		);
		...
		const totalT5Kills = stats.reduce(
			(sum, stat) => sum + (stat.killcount_t5 || 0),
			0
		);
		const totalT1Kills = stats.reduce(
			(sum, stat) => sum + (stat.killcount_t1 || 0),
			0
		);
		const totalManaSpent = stats.reduce(
			(sum, stat) => sum + (stat.mana_spent || 0),
			0
		);
		const totalUnitsDead = stats.reduce(
			(sum, stat) => sum + (stat.units_dead || 0),
			0
		);
		const totalUnitsHealed = stats.reduce(
			(sum, stat) => sum + (stat.units_healed || 0),
			0
		);
		const totalHelpsGiven = stats.reduce(
			(sum, stat) => sum + (stat.helps_given || 0),
			0
		);
		const totalScouted = stats.reduce(
			(sum, stat) => sum + (stat.scouted || 0),
			0
		);
		const averagePower =
			memberCount > 0 ? Math.round(totalPower / memberCount) : 0;
		...
		rankingsData.push({
			...
			totalHelpsGiven,
			totalScouted,
			lastScanDate: statsResult.scanDate,
		});
	}
...
export async function fetchPlayerTimeline(
	...
	// Format stats with scan_date
	const formattedStats: Stat[] = (stats || []).map((s: any) => ({
		...s,
		scan_date: s.scans?.scan_date,
	}));
