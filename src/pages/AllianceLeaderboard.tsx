...
import { useAllianceRankings } from "../hooks/useAllianceRankings";
...
export function AllianceLeaderboard() {
	...
	const [activeCategory, setActiveCategory] =
		useState<RankingCategory>("totalPower");

	const categories = [
		{
			id: "totalPower" as const,
			name: "Total Power",
			icon: TrendingUp,
			color: "text-blue-400",
		},
		{
			id: "averagePower" as const,
			name: "Average Power",
			icon: Crown,
			color: "text-yellow-400",
		},
		{
			id: "totalMerits" as const,
			name: "Total Merits",
			icon: Award,
			color: "text-purple-400",
		},
		{
			id: "totalKills" as const,
			name: "Total Kills",
			icon: Sword,
			color: "text-red-400",
		},
		{
			id: "totalT5Kills" as const,
			name: "T5 Kills",
			icon: Target,
			color: "text-orange-400",
		},
		{
			id: "totalT1Kills" as const,
			name: "T1 Kills",
			icon: Target,
			color: "text-green-400",
		},
		{
			id: "totalManaSpent" as const,
			name: "Mana Spent",
			icon: Zap,
			color: "text-cyan-400",
		},
		...
		return (
			...
			>
				<div className="text-center">
					...
					<p className="text-yellow-400 font-orbitron font-bold">
				...
				<div className="p-4 sm:p-6 border-b border-gray-700/50 relative z-10">
					<div className="flex items-center justify-between">
						<div className="flex items-center space-x-3">
							{activeConfig && (
								<activeConfig.icon
									className={`w-6 h-6 ${activeConfig.color}`}
								/>
							)}
						...
						{sortedRankings.length > 0 ? (
										...
										>
											<div className="flex items-center space-x-3 sm:space-x-4 flex-1 min-w-0">
												...
												<div className="flex items-center space-x-2">
													{getRankIcon(rank)}
													<span
														className={`font-bold font-orbitron text-sm sm:text-base ${
															rank === 1
																? "text-yellow-400"
																: rank === 2
																? "text-gray-300"
											...
											<div className="text-right flex-shrink-0">
												<div
													className={`text-lg sm:text-xl md:text-2xl font-black font-orbitron ${
														rank === 1
															? "text-yellow-400"
															: rank === 2
															? "text-gray-300"
							...
							<div className="flex items-center justify-center py-12">
								<div className="text-center">
									...
									<h3 className="text-lg font-bold text-gray-300 font-orbitron mb-2">
										NO RANKINGS AVAILABLE
									</h3>
									<p className="text-gray-400">
										No alliance data found for rankings
									</p>
								</div>
							</div>
						)}
			...
			>
				...
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 relative z-10">
					...
					<div className="text-center p-4 glass-panel-light rounded-xl neon-border">
						...
						<div className="text-xl sm:text-2xl font-black text-white font-orbitron">
							{formatNumber(
								rankings.reduce((sum, a) => sum + (a.memberCount || 0), 0)
							)}
						</div>
					</div>
